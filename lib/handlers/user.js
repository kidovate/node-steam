...
Steam.randomServer = function()
{
  return Steam.servers[Math.floor(Math.random() * Steam.servers.length)];
};

prototype.logOn = function(logOnDetails, options) {
  this.logOff();
  ...
  var server = Steam.randomServer();
  var doConnect = false;

  if(this._options.customSocket && this._options.socketConnected)
  {
    this.emit('debug', 'using existing connection');
    this._connection = (require('../preconnection')(this._options.customSocket));
  }else
  {
    doConnect = true;

    this.emit('debug', 'connecting to ' + server.host + ':' + server.port);
    this._connection = new (require('../connection')(this._options.customSocket))();
...
prototype.webLogOn = function(callback) {
  ...
  this._serverPubKey = ursa.createPublicKey(fs.readFileSync(__dirname + '/../public.pub'));
  var cryptedSessionKey = this._serverPubKey.encrypt(sessionKey);
